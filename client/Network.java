            ...
            Thread clientListenerThread = new Thread(() -> {
                try {
                    ...
                    callOnException.callback("Соединение с сервером разорвано");
                                                                        //пытался тут как то сокет закрыть не получилось

                } finally {
                    close();
                    System.exit(0);    //не знаю насколько правильно я это делаю не мог понять как правильно закрыть сокет socket.setSoTimeout выбрасывает исключение но не закрывает
                }
            });
